...
from .evaluator_registry import materialize_git_checkout, validate_git_checkout
...
async def solve(request: SolveRequest) -> SolveResult:
    ...
    if existing is not None:
        _agent_spec, _deployer_cls, config = _resolve_requested_agent(request)
    ...
    with materialize_git_checkout(
        ...
        request.task_commit,
        category="task_checkout",
        prefix="ale-solve-checkout-",
    ) as checkout:
        runtime_request = _request_for_checkout(request, checkout)
        agent_spec, deployer_cls, config = _resolve_requested_agent(runtime_request)
        config_digest = _agent_config_digest(config)
        with tempfile.TemporaryDirectory(prefix="ale-atomic-solve-") as work_dir:
            async with AtomicRuntime.open(request=runtime_request) as runtime:
                executor_type = (
                    agent_spec.executor
...
def _request_for_checkout(request: SolveRequest, checkout: Path) -> SolveRequest:
    ...
    try:
        ...
        updates["runtime_spec_path"] = checkout / runtime_spec_relative
